...
const Expenses = () => {
  ...
  const categories = ['All', 'Raw Materials', 'Packaging', 'Labour', 'Transportation', 'Electricity & Gas', 'Marketing'];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-orange-50/70 border border-orange-200/80 rounded-3xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-orange-700 mt-0.5">{expenses.length} expense items recorded</p>
        </div>
              ...
              <tbody className="divide-y divide-slate-100 font-medium text-slate-600">
                {filteredExpenses.map((exp) => (
                  <tr key={exp._id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="p-4 text-right font-black text-slate-900">{formatCurrency(exp.amount)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            >
              ...
              <option value="Labour">Labour / Making</option>
