            ...
            <Link to="/" className="flex items-center gap-2.5 group">
              ...
              <div className="flex flex-col">
                <span className="font-extrabold text-xl leading-none text-slate-900 tracking-tight flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] bg-orange-100 text-orange-700 px-1.5 py-0.5 rounded-full font-bold uppercase tracking-wider">SIH</span>
                </span>
          ...
          <div className="hidden md:flex items-center">
            {business ? (
              <div className="flex items-center gap-2 px-3 py-1 bg-amber-50 border border-amber-200/80 rounded-full text-xs font-semibold text-amber-900">
                ...
                <span className="text-amber-600">({business.state || 'AP'})</span>
              </div>
            ) : (
              <Link
              ...
              >
                ...
                <span>{t.nav?.discovery || 'Business Discovery'}</span>
              </Link>
            )}
          </div>
                ...
                {profileDropdownOpen && (
                  <div className="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-xl border border-slate-100 py-2 z-50 animate-in fade-in slide-in-from-top-2">
                    <div className="px-4 py-2 border-b border-slate-100">
                      ...
                      <p className="text-sm font-bold text-slate-900 truncate">{user?.name || 'Lakshmi Devi'}</p>
                      <p className="text-xs text-orange-600 font-medium">{business?.name || 'Lakshmi Foods'}</p>
                    </div>
                    ...
                    >
                      ...
                      {t.nav?.profile || 'Business Profile'}
                    </Link>
